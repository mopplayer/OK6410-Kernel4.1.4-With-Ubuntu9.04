...
if(!defined (defined(&_SIGNAL_H) ? &_SIGNAL_H : undef)  && !defined (defined(&__need_siginfo_t) ? &__need_siginfo_t : undef)  && !defined (defined(&__need_sigevent_t) ? &__need_sigevent_t : undef)) {
    die("Never include this file directly.  Use <signal.h> instead");
}
require 'bits/wordsize.ph';
if((!defined (defined(&__have_sigval_t) ? &__have_sigval_t : undef)  && (defined (defined(&_SIGNAL_H) ? &_SIGNAL_H : undef) || defined (defined(&__need_siginfo_t) ? &__need_siginfo_t : undef) || defined (defined(&__need_sigevent_t) ? &__need_sigevent_t : undef)))) {
    eval 'sub __have_sigval_t () {1;}' unless defined(&__have_sigval_t);
}
if((!defined (defined(&__have_siginfo_t) ? &__have_siginfo_t : undef)  && (defined (defined(&_SIGNAL_H) ? &_SIGNAL_H : undef) || defined (defined(&__need_siginfo_t) ? &__need_siginfo_t : undef)))) {
...
if((defined (defined(&_SIGNAL_H) ? &_SIGNAL_H : undef) || defined (defined(&__need_sigevent_t) ? &__need_sigevent_t : undef))  && !defined (defined(&__have_sigevent_t) ? &__have_sigevent_t : undef)) {
